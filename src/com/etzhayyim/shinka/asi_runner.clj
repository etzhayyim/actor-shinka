...
(defn fetch-fleet
  "Read the public cloud-murakumo capacity snapshot."
  ([] (fetch-fleet (or (System/getenv "MURAKUMO_CLOUD") default-cloud-url)))
  ([base-url]
   (let [resp (knet/http-get (str base-url "/infer/fleet"))]
     (when (and resp (= 200 (:status resp)))
       (json/parse-string (:body resp) true)))))

(defn fetch-models
  "Read the public serving-model catalog."
  ([] (fetch-models (or (System/getenv "MURAKUMO_CLOUD") default-cloud-url)))
  ([base-url]
   (let [resp (knet/http-get (str base-url "/infer/models"))]
     (when (and resp (= 200 (:status resp)))
       (json/parse-string (:body resp) true)))))
...
(defn fetch-runtime-models
  "Read the gateway's own `/v1/models` response."
  [endpoint]
  (let [url (gateway-models-url endpoint)
        resp (knet/http-get url)]
    (when (and resp (= 200 (:status resp)))
